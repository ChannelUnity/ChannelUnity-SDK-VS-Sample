...
namespace CUSDK
{
    ...
    public class ConnectorKit
    {
        ...
        /// Request type.
        /// </param>
        private string PostToChannelUnity(string payload, string requestType, string urlOverride = null)
        {
            string urlToUse = CU_ENDPOINT;
            if (urlOverride != null)
            {
                urlToUse = urlOverride;
            }

            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(urlToUse);
            request.Method = "POST";
            // Don't allow automatic redirects because the POST gets changed to a GET
            ...
            if (value == "")
            {
                // Request failed, let's see if we have been given a Location header
                ...
                if (newLocation != null)
                {
                    // There is a location header - let's do our request again with the new location
                    Console.WriteLine("Repeat the request with URL " + newLocation);
                    return this.PostToChannelUnity(payload, requestType, newLocation);
                }
            }
